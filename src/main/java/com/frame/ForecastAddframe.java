...
public class ForecastAddframe extends JDialog implements Runnable, ActionListener {// 随机选号对话框
	...
	public ForecastAddframe() {// 随机选号对话框的构造方法
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(ForecastAddframe.class.getResource("/imgs/log.png")));// 设置随机选号对话框的标题图标
		...
		but.setIcon(new ImageIcon(ForecastAddframe.class.getResource("/img_btn/111.png")));// 设置“机选一注”按钮的图标
		...
		btnNewButton.setIcon(new ImageIcon(ForecastAddframe.class.getResource("/img_btn/a02.png")));// 设置“购买”按钮的图标
		...
		btnNewButton_1.setIcon(new ImageIcon(ForecastAddframe.class.getResource("/img_btn/a07.png")));// 设置“关闭”按钮的图标
	...
	protected void do_rabtnNone_actionPerformed(ActionEvent e) {// 单选按钮“无”动作事件的监听
		// 复选框没被选中
	...
	public void run() {// 线程需要完成的任务
		...
		if (rdbtnHave.isSelected() && (exchangeInteger(tf_1) + exchangeInteger(tf_2) + exchangeInteger(tf_3)
				+ exchangeInteger(tf_4) + exchangeInteger(tf_5) + exchangeInteger(tf_6) + exchangeInteger(tf_7)
				+ exchangeInteger(tf_8) + exchangeInteger(tf_9) + exchangeInteger(tf_0)) > 7) {
			JOptionPane.showMessageDialog(null, "警告：号码个数多于7个！", "警告", JOptionPane.WARNING_MESSAGE);// 弹出提示框
			but.setEnabled(true);// 设置“机选一注”按钮可用
			return;
		}
		String s = "";// 存储随机生成的选号
			...
			while (bol) {
				try {
					...
					if (index < (1000 - ram.nextInt(20))) {// 控制停止时间1000毫秒减去20以内的随机数
						b = i;// 获得随机选号第2位的数字
						bt2.setIcon(new ImageIcon(ForecastAddframe.class.getResource("/imgs/" + i + ".png")));// 通过循环变换图片以达到随机选号第2位的摇奖结果
					}
					if (index < (1500 - ram.nextInt(20))) {// 控制停止时间1500毫秒减去20以内的随机数
						c = i;// 获得随机选号第3位的数字
						bt3.setIcon(new ImageIcon(ForecastAddframe.class.getResource("/imgs/" + i + ".png")));// 通过循环变换图片以达到随机选号第3位的摇奖结果
					}
					if (index < (2000 - ram.nextInt(20))) {// 控制停止时间2000毫秒减去20以内的随机数
						d = i;// 获得随机选号第4位的数字
						bt4.setIcon(new ImageIcon(ForecastAddframe.class.getResource("/imgs/" + i + ".png")));// 通过循环变换图片以达到随机选号第4位的摇奖结果
					}
					if (index < (3000 - ram.nextInt(20))) {// 控制停止时间3000毫秒减去20以内的随机数
						e = i;// 获得随机选号第5位的数字
						bt5.setIcon(new ImageIcon(ForecastAddframe.class.getResource("/imgs/" + i + ".png")));// 通过循环变换图片以达到随机选号第5位的摇奖结果
					}
					if (index < (4000 - ram.nextInt(20))) {// 控制停止时间4000毫秒减去20以内的随机数
						f = i;// 获得随机选号第6位的数字
						bt6.setIcon(new ImageIcon(ForecastAddframe.class.getResource("/imgs/" + i + ".png")));// 通过循环变换图片以达到随机选号第6位的摇奖结果
					}
					if (index < (5000 - ram.nextInt(20))) {// 控制停止时间5000毫秒减去20以内的随机数
						g = i;// 获得随机选号第7位的数字
						bt7.setIcon(new ImageIcon(ForecastAddframe.class.getResource("/imgs/" + i + ".png")));// 通过循环变换图片以达到随机选号第7位的摇奖结果
					}
	...
	private int exchangeInteger(JTextField tf) {// 将文本框的内容转换为int型
	...
	protected void do_btnNewButton_1_actionPerformed(ActionEvent e) {// “关闭”按钮动作事件的监听
		this.setVisible(false);// 使随机选号对话框不可见
	}

	protected void do_btnNewButton_actionPerformed(ActionEvent e) {// “购买”添加动作事件的监听
		LotteryValidate validate = new LotteryValidate();// 实例化验证信息
		if (!validate.validateNumber(sevenTextField.getText())) {// “选号号码”本框输入的“选号号码”的格式错误
			noteLabel.setText("购买号码格式不正确");// 设置“提示”标签内容
		}
		if (validate.validateNumber(sevenTextField.getText())) {// “选号号码”本框输入的“选号号码”的格式正确
			noteLabel.setText("");// 设置“提示”标签内容为空
			ForecastDao fr = new ForecastDao();// 实例化操作购买彩票记录信息
			Boolean b;
			b = fr.addForecastDao(Integer.parseInt(numberTextField.getText()), sevenTextField.getText());// 获得添加购买号码的返回值
			if (b) {// 添加购买号码成功
